...
pub fn stage_one(html_response: &str) {
    //let rset = RegexSet::new(&[r"yo",r"ni(.*?)"]);
    //let yo = rset.unwrap().captures_iter();
    ...
    let title_expression = Regex::new(r#"</div><div class="(?:.*?)"><div class="(?:.*?)"><div class="(?:.*?)">(.*?)</div><div class="(?:.*?)"><span id="(?:.*?)" class="(?:.*?)"><div class="(?:.*?)">(?:.*?)<img src="(?:.*?)" class="(?:.*?)"></div></span>"#).unwrap();
    ...
    let title: String = title_expression.captures_iter(html_response).map(|capture| capture[1].to_string()).collect();
    println!("Found:TITLE: {} REST {:#?}", title, listing_url);
